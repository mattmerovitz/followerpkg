...
class Follower:
    ...
    # Compute the "centroid" and display a red circle to denote it
        ...
        if M['m00'] > 0:
            ...
            if self.followingline: # If it was already following the line, just turn around and keep following
                self.twist.linear.x = 0
                self.twist.angular.z = 0.8

            else: # If not, turn randomly until line is found.
                self.twist.linear.x = 0.2
                self.twist.angular.z = random.uniform(0.1,0.3)            
